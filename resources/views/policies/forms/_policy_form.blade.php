    ...
    <form id="policy-form" action="{{ $formAction }}" method="POST" class="space-y-4 p-4 rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="date" id="issue_date" name="issue_date" value="{{ $policy->issue_date }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-red-600 focus:border-red-600 block w-full p-2.5" required>
            </div>
            <div>
                <label for="effective_date" class="block text-sm font-medium text-gray-700">Effective Date</label>
                <input type="date" id="effective_date" name="effective_date" value="{{ $policy->effective_date }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-red-600 focus:border-red-600 block w-full p-2.5" required>
            </div>
            <div>
                <label for="expiry_date" class="block text-sm font-medium text-gray-700">Expiry Date</label>
                <input type="date" id="expiry_date" name="expiry_date" value="{{ $policy->expiry_date }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-red-600 focus:border-red-600 block w-full p-2.5" required>
            </div>
        </div>
        ...
        <div>
            ...
            <div id="coverage-details-container" class="space-y-4">
                <!-- Dynamic entries will be added here via JS -->
            </div>
            ...
            <input type="hidden" name="coverage_details" id="coverage_details_hidden" value="{{ json_encode($coverageDetails) }}">
...
<script>
    // JS for dynamic coverage details (add as needed)
    document.getElementById('add-coverage-detail').addEventListener('click', function() {
        // Add dynamic field logic here, e.g., append input pairs and update hidden field on change
    });
</script>