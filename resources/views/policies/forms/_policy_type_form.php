...
$formAction = $isEdit ? '/admin/policies/updatePolicyType' : '/admin/policies/storePolicyType';
$dialogTitle = $isEdit ? 'Edit Policy Type' : 'Add New Policy Type';

// Default values for new policy type
...
<section class="flex-1 bg-gray-50 p-4 shadow-xs rounded-md overflow-hidden h-full">
    ...
    <div class="mt-6 p-6 rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <form action="<?= $formAction ?>" method="POST" class="space-y-4">
            <?php if ($isEdit): ?>
                <input type="hidden" name="id" value="<?= htmlspecialchars($policyType->id) ?>">
            <?php endif; ?>

            <div>
                <label for="type_name" class="block text-sm font-medium text-gray-700">Policy Type Name</label>
                <input type="text" id="type_name" name="type_name" value="<?= htmlspecialchars($policyType->type_name) ?>"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-red-600 focus:border-red-600 block w-full p-2.5"><?= htmlspecialchars($policyType->description ?? '') ?></textarea>
            </div>
